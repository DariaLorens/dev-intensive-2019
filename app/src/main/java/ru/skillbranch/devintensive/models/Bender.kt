...
class Bender(var status: Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    private fun validateAnswer(answer: String): Pair<Boolean, String> {
        return when {
            question == Question.NAME && answer.substring(0, 1).toUpperCase() != answer.substring(
                0,
                1
            ) -> false to "Имя должно начинаться с заглавной буквы"
            question == Question.PROFESSION && answer.substring(0, 1).toLowerCase() != answer.substring(
                0,
                1
